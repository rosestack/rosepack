...
class Rosepack {
    ...
    if (this.packageJson) {
      ...
      if (this.config.run) {
        if (!this.packageJson.main) {
          throw new Error(`${this.logger.mark("package.json")} has no ${this.logger.mark("main")} field`);
        }
      }
      ...
      if (this.config.externalDeps === null) {
        this.config.externalDeps = this.config.target === "node";
      }

      if (this.config.externalPeerDeps === null) {
        this.config.externalPeerDeps = this.config.target === "node";
      }

      if (this.config.externalDevDeps === null) {
        this.config.externalDevDeps = this.config.target === "node";
      }

      if (this.config.externalDeps) {
        this.config.externalDeps = Object.keys(this.packageJson.dependencies ?? {});
      }

      if (this.config.externalDevDeps) {
        this.config.externalDevDeps = Object.keys(this.packageJson.devDependencies ?? {});
      }

      if (this.config.externalPeerDeps) {
        this.config.externalPeerDeps = Object.keys(this.packageJson.peerDependencies ?? {});
      }
  ...
  private loadEnv = async (main: boolean) => {
    ...
    this.logger.debug("[define]", util.inspect(this.config.define, {
      depth: Infinity,
      colors: true
    }));
    this.logger.debug("[defineEnv]", util.inspect(this.config.defineEnv, {
      depth: Infinity,
      colors: true
    }));
    ...
    if (this.config.createEnv) {
      ...
      let env = fs.readFileSync(envTemplate, "utf-8");
    ...
    const promises = copies.map(async (copy) => {
      ...
      if (glob.isDynamicPattern(from)) {
        ...
        for (const file of files) {
          ...
          fs.copyFileSync(file, absolutePath);
  ...
  private runTasks = async () => {
    ...
    this.tasks = formats.map((format) => {
      return new Task({
        ...
        noExternal: this.config.noExternal,
        externalDeps: this.config.externalDeps as string[],
        externalDevDeps: this.config.externalDevDeps as string[],
        externalPeerDeps: this.config.externalPeerDeps as string[],
        //
