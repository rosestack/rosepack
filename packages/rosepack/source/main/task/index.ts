...
class Task {
  ...
  private commonRollup = () => {
    ...
    const external = createFilter({
      include: this.config.external,
      exclude: this.config.noExternal,
      default: true
    });

    external.include([
      ...this.config.externalDeps ?? [],
      ...this.config.externalDevDeps ?? [],
      ...this.config.externalPeerDeps ?? []
    ]);
    ...
    return merge(this.config.rollupOptions ?? {}, {
      ...
      external: (source, importer) => {
        ...
        if (this.config.tsConfig?.compilerOptions) {
          const hasMatchingPath = Object.keys(this.config.tsConfig?.compilerOptions.paths ?? {}).some((path) => {
            return new RegExp("^" + (path.replace("*", ".+")).replace(/[$^]/g, "\\$&") + "$").test(source);
          });
        ...
        const result = external(moduleName);
  ...
  private commonSwc = () => {
    ...
    return merge(this.config.swcOptions ?? {}, {
      cwd: this.config.cwd,
  ...
  private nodemon = () => {
    ...
    this.child.on("spawn", () => {
      this.logger.info(`Running ${this.logger.mark(this.config.packageJson.main)}`);
    });

    this.child.on("exit", (code) => {
      this.logger.info("Exited with code", this.logger.mark(code));
      this.child = undefined;
    });
  };
